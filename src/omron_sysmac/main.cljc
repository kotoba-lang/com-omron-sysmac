(ns omron-sysmac.main
  "Kotodama WASM entrypoint for the omron_sysmac clean-room actor (L4) — Clojure port.
  ...
  facts (`omron_sysmac.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "omron_sysmac")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "omronsys_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "omronsys_con"
    :fields [:name :setpoint :processValue :mode]      :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}    :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "omronsys_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"omron_sysmac.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "omron_sysmac-compat" :tier tier :entities entities} 200])
...
(defn start! [] :omron_sysmac-compat/ready)
