(ns omron-sysmac.main-test
  "Contract + behavioral test for the omron_sysmac-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "omron_sysmac-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'omron-sysmac.main-test)]
